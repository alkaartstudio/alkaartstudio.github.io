...
/**
 * Loads the initial set of data for the home page (featured gallery items and first 2 courses).
 * * UPDATED: Now uses the full multi-image slider structure for course cards.
 */
async function loadAndFilterHomePage() {
    ...
    if (homeCoursesGrid) {
        publishedCourses.slice(0, 2).forEach(course => { 
            const whatsappLink = course.whatsapp_link;
